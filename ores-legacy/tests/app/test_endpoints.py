...
@pytest.mark.asyncio
async def test_get_model_scores_returns_404_when_context_does_not_exist(client):
    response = await client.get("/v3/scores/commonswiki/800966850/articletopic")
    assert response.status_code == 404
...
@pytest.mark.asyncio
async def test_get_model_scores_400_wrong_model(client):
    response = await client.get("/v3/scores/enwiki/12345/reverted?features=true")
    assert response.status_code == 400
